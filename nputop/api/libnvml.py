...
from nputop.api.utils import NA, UINT_MAX, ULONGLONG_MAX, NaType
from nputop.api.utils import colored as __colored
...
if not callable(getattr(_pynvml, 'nvmlInitWithFlags', None)):
    raise ImportError(  # noqa: TRY004
        ...
        '`nvidia-ml-py` via `pip3 install --force-reinstall nvidia-ml-py nputop`.',
    )
...
if not LOGGER.hasHandlers() and LOGGER.isEnabledFor(_logging.DEBUG):
    ...
    _file_handler = _logging.FileHandler('nputop.log')
...
def nvmlInitWithFlags(flags: int) -> None:  # pylint: disable=function-redefined
    ...
    try:
        ...
        message = (
            ...
            'Please reinstall `nputop` with command:\n'
            '    python3 -m pip install --force-reinstall nputop'
        )
        for text, color, attrs in (
            ('FATAL ERROR:', 'red', ('bold',)),
...
def nvmlCheckReturn(
    ...
    """Check whether the return value is not :const:`nputop.NA` and is one of the given types."""
    if types is None:
        return retval != NA
